...
export default LanguageScreen = () => {
  ...
  const getAllData = async () =>{
      const snapshot = await firebase.firestore().collection('toDoLanguage').onSnapshot(snapshot=>{
...
const updateItem = async (id) => {
  ...
  >
    ...
    padding:5, 
    borderRadius:8}}  onPress={() => navigation.navigate('Language')}>
          <Text style={{paddingHorizontal:5, 
    fontSize:19,
    color:theme.background}}>Language</Text>
        </TouchableOpacity>
      </View>
      <View style={styles.filterBar}>
        <TouchableOpacity style={styles.filterButton} onPress={() => navigation.navigate('Humanities')}>
          <Text style={styles.filterText}>Humanities</Text>
        </TouchableOpacity>
      </View>
      <View style={styles.filterBar}>
