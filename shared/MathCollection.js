...
export default MathScreen = () => {
  ...
  const getAllData = async () =>{
      const snapshot = await firebase.firestore().collection('toDoMath').onSnapshot(snapshot=>{
...
const updateItem = async (id) => {
  ...
  >
    ...
    padding:5, 
    borderRadius:8}}  onPress={() => navigation.navigate('Math')}>
          <Text style={{paddingHorizontal:5, 
    fontSize:19,
    color:theme.background}}>Math</Text>
        </TouchableOpacity>
      </View>
      <View style={styles.filterBar}>
